  ...
  return (
    <section className="relative  flex items-center justify-center">
      ...
      <div className='absolute -z-10 inset-0 bg-white'>
        ...
        <div className='h-full w-full grid grid-cols-[repeat(8,1fr)] grid-rows-[repeat(12,1fr)] sm:grid-cols-[repeat(12,1fr)] sm:grid-rows-[repeat(14,1fr)] md:grid-cols-[repeat(16,1fr)] md:grid-rows-[repeat(15,1fr)] lg:grid-cols-[repeat(20,1fr)] lg:grid-rows-[repeat(15,1fr)]'>
          ...
          {Array.from({ length: 240 }).map((_, i) => (
            <div 
              key={`md-${i}`} 
              className='border border-black/10 hidden md:block lg:hidden'
            />
          ))}
          {Array.from({ length: 168 }).map((_, i) => (
            <div 
              key={`sm-${i}`} 
              className='border border-black/10 hidden sm:block md:hidden'
            />
          ))}
          {Array.from({ length: 96 }).map((_, i) => (
            <div 
              key={`mobile-${i}`} 
              className='border border-black/10 block sm:hidden'
            />
          ))}
        </div>
      </div>
